...
See [[current-state]]. Sprint 2 complete. Sprint 3 deterministic allocation is IN PROGRESS with constraints registry, scored `find_feasible_slots` ranking, `request_slot`, `get_appointment_request_status`, live same-slot contention proof, allocation unique-index race mapping, Redis memory tool context, individual POC Auth users, and polished role-specific UI in place. **OpenAI + OpenRouter live invoke verified; current Gemini direct REST verified with `gemini-flash-latest`, while current LangChain Gemini invoke needs restart/recheck.**
