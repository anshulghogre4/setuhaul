...
See [[current-state]]. Sprint 2 complete. Sprint 3 deterministic allocation is IN PROGRESS with constraints registry, `find_feasible_slots`, `request_slot`, `get_appointment_request_status`, and allocation unique-index race mapping in place. **OpenAI + OpenRouter + Gemini live invoke verified.**
...
- Backend scheduling/LangChain allocation path: `$env:PYTHONPATH=(Get-Location).Path; uv --system-certs run --with pytest pytest tests\unit` from `backend/` PASS: 38 passed, 1 pytest config warning (`asyncio_mode` unknown without `pytest-asyncio` in ephemeral env). FastAPI import smoke PASS with 11 routes.
