...
- **False-escalation bug — three fix layers now, deploy still owed (2026-08-17 08:10 IST)** — live driver-chat testing (owner, hosted screenshots) caught `escalate_exception` firing on the plain context-lock line `I need help with shipment X`, creating a real `OPEN`/`HIGH` escalation (`ESC-53B8A6EA0A37` for `SHP-D16-RAVI` at `FAC-JAI-01`). Layer 1 (06:35 IST): prompt wording fixed to gate escalation on `NO_FEASIBLE_SLOTS`/explicit ask. Layer 2 (07:20→07:35 IST): first redeploy silently shipped stale code because `agentcore/codezip/` wasn't re-staged before `agentcore.cmd deploy`; re-staged, verified via downloading the live S3 artifact zip that the fix genuinely deployed (`agentRuntimeVersion=6`). **Bug still reproduced live anyway on a fresh session** — the LLM (`gpt-4o-mini`) still misjudges intent, likely blending the context-lock line with the immediately-preceding delay/ETA-report turn; prompt wording alone is now confirmed insufficient. Layer 3 (08:10 IST): `escalate_exception` made a two-step confirm write (mirrors `report_delay_or_update_eta`) — `EscalateExceptionCommand.confirmed` defaults `True` for existing system/ops callers, the driver-chat tool's arg defaults `False`, so a misfired call now returns `CONFIRMATION_REQUIRED` with zero DB writes instead of a real escalation. Backend units 87 passed; codezip re-staged again. **Not yet redeployed to either hosted target or live-verified** — needs `agentcore.cmd deploy --yes` and an ECS image rebuild/roll. The stray `ESC-53B8A6EA0A37` escalation record still has not been resolved/deleted.
