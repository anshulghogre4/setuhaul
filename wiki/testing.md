...
- `reschedule_appointment` stale-hash fix + isolated sandbox driver: **PASS** (2026-08-16 21:30 IST). Live on Supabase: seed via `seed_reschedule_driver.py` produced 2× `SLOT_REQUESTED` + 1× `APPOINTMENT_CONFIRMED` through the real `request_slot`/`confirm_appointment` services; isolation check confirmed zero `FAC-JAI-01` writes, `D16-SLT-RACE` free, all cast shipments still with no active appointment, total shipments 667→671 (+4 exactly). Reschedule smoke on `SHP-RS-PENDING` and `SHP-RS-CONFIRMED` failed `SLOT_OPTIONS_STALE` before the fix (2/2, reproducible), passed `SLOT_REQUESTED` with correct `replaced_appointment_id` chain after the fix (2/2); negative test with a bogus `recommendation_id` still correctly returned `SLOT_OPTIONS_STALE`. Backend unit `pytest tests/unit -q` **81 passed**. Live `SETUHAUL_RUN_LIVE_DB_TESTS=1 pytest tests/integration/test_live_demo_day_load.py -q` **2 passed** (10×4 scarce load unaffected: 4 winners/6 conflicts/zero double-books; D16 cast smoke unaffected).
- `reset_demo_day.py --mode full` FK-crash fix: **verified via dry-run** (2026-08-16 21:30 IST). Before the fix, `--dry-run` preview showed a misleading `shipments_SHP_D16_kept_non_cast_referenced=574` (queried pre-transaction state rather than simulating the prior same-transaction delete); corrected to `0` after fixing the preview query to match real execution order, and cross-checked against a direct simulation query. Confirmed a genuinely surviving reference exists today independent of the new sandbox (`APT-F16FD7C41E41` / `SHP-DISP-83C3F2C0` / slot `D16-SLT-02739`, a Dispatch Console auto-booking) — `slots_D16_kept_non_cast_referenced=4` in the corrected dry-run. `--mode cast --include-shp1017 --dry-run` re-run unaffected (same counts as before the edit). `--mode full --confirm` was **not** run live (rare/destructive, not requested).
