...
test('admin: tabs and rail', async ({ page }) => {
  ...
  say(
    ...
    'MISSING',
    `the rail renders ${railCount} destination (the admin console) and no Profile entry -- icon-rail.tsx emits exactly one railDestinationFor(role). SEPARATE OBSERVATION worth the owner's attention: the design says this surface has NO facility switcher ("admin scope is set per action, not by a global view filter"), but one IS rendered in the top bar, labelled "${switcherLabel}" -- hasFacilityScope('ADMIN') is true while a global admin's identity.facilities is empty, so it is an empty picker.`,
  )
