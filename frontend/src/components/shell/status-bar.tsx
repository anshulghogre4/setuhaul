...
export function StatusBar({
  ...
  const { icon: Icon, label, danger, spin } = CONNECTION[connection]
  ...
  return (
    <div className="flex h-7 w-full shrink-0 items-center gap-3 border-t border-input bg-card px-3 text-[11px] leading-[1.3] text-muted-foreground">
      ...
      <span className="inline-flex shrink-0 items-center gap-1.5 whitespace-nowrap">
        Last sync <span className="font-mono" translate="no">{lastSync}</span>
      </span>
      ...
      <Separator />
      <span className="inline-flex shrink-0 items-center gap-1.5 whitespace-nowrap">
        <span className="font-mono" translate="no">{pendingCount}</span> pending
      </span>
