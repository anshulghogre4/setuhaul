/**
 * Kibo UI Gantt -- vendored SOURCE (MIT), installed via `npx kibo-ui add gantt`, not npm.
 * Installed in E5.0 per the tech-stack sub-issue; CONSUMED by E5.3 (planner dock board).
 *
 * Four limits, verified against THIS file rather than the published docs (which mention
 * neither zoom nor virtualisation).  Read them before planning E5.3:
 *
 *  1. `Range = "daily" | "monthly" | "quarterly"` (line ~86).  The finest grid is ONE COLUMN
 *     PER DAY.  There is no hourly grid.  A dock board is an intra-day instrument -- 6-12
 *     lanes across ~06:00-22:00 at 30-90 minute intervals -- so getting a usable board means
 *     owning a custom time-axis header, not configuration.  Item positioning within a column
 *     does use `differenceInHours`, so a bar CAN sit at an hour offset; the header still
 *     labels days.
 *  2. `zoom: number`, default 100, a unitless percentage.  **No presets.**  Any preset layer
 *     is ours to build.
 *  3. **No virtualisation at all** -- no react-window, no @tanstack/react-virtual anywhere in
 *     this file.  Every column and every bar renders into the DOM.
 *  4. **Drag/resize is opt-in**: the handles render only inside `{onMove && (...)}`.  This is
 *     the good news -- U25 ("no free dragging") is satisfied by simply NOT passing `onMove`.
 *     Kibo's headline feature is exactly what our design forbids, and it is off by default.
 *     `GanttFeatureDragHelper` and `GanttAddFeatureHelper` are exported from this file and
 *     should stay unused; the spec asked for them to be uninstalled, which is not separable
 *     here because Kibo ships one file.
 *
 * Whether this is a better trade than a purpose-built lane renderer over
 * @tanstack/react-virtual is a real question for E5.3's planning, to be asked THEN with the
 * dock board's actual requirements in hand. U52 stays open -- now open with specifics.
 */
"use client";
...
export const GanttCreateMarkerTrigger: FC<GanttCreateMarkerTriggerProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        <button
          className="z-dropdown inline-flex h-4 w-4 items-center justify-center rounded-full bg-card"
          onClick={handleClick}
          type="button"
        >
...
export const GanttFeatureItemCard: FC<GanttFeatureItemCardProps> = ({
  ...
  return (
    <Card className="h-full w-full rounded-md bg-background p-2 text-xs shadow-raised">
      <div
        className={cn(
