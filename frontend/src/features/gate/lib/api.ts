...
export async function updateQueueState(
  ...
  const res = await apiPost<GateEventResult>(
    ...
    { queue_state: queueState, queue_position: null },
  )
  return res.data
}
...
export async function recordDockIn(shipmentId: string, dockId: string): Promise<GateEventResult> {
  const res = await apiPost<GateEventResult>(
...
export async function recordGateOut(shipmentId: string): Promise<GateEventResult> {
  const res = await apiPost<GateEventResult>(
