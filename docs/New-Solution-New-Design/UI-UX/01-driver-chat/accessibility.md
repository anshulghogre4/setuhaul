...
| **Promise-state chips use the `filled` variant here, always** | **Rationale corrected 2026-08-27 (E5.1) — the conclusion stands, the reason given for it did not.** This row used to read *"an outline chip loses its signal under glare — the fill is what survives."* Measured in a real render, the fill is the **first** channel to go, not the last: the filled backgrounds sit at **1.00–1.04:1** against `surface-base`, and a sweep of every step of all four ramps found **no tint** that clears 3:1 while keeping 4.5:1 text — amber has no such step at all. Glare compresses contrast toward white, and a 1.01:1 tint has nothing left to lose. What actually survives is the **border *treatment*** (dashed vs 2px-solid vs 1px-solid is a *shape* difference, immune to contrast compression), the **icon shape**, and the **label text** at 4.84–9.90:1. `filled` stays mandatory here because a visible tint still helps in ordinary light and costs nothing — it is just not the load-bearing channel. Two borders were raised one ramp step in the same pass so all four states clear WCAG 1.4.11 (`shown` `neutral-300 → neutral-500`, 1.42 → 4.55; `held` `amber-500 → amber-600`, 2.05 → 3.04) |
