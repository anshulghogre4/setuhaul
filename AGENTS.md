...
- **Any AgentCore Runtime deploy goes through `python docs/scripts/agentcore_deploy.py`, never `agentcore deploy` directly.** (E4.2/issue #32, 2026-08-26.) The wrapper stages `agentcore/codezip/` from `backend/app/` + `backend/pyproject.toml`, runs the backend test suite plus `agentcore package` as a local-verify gate, then deploys, then compares `agentcore/.cli/deployed-state.json`'s `deployHash` before/after — a real content-change signal, not a trusted exit code. `agentcore deploy` packages `agentcore/codezip/app/`, a separate copied snapshot of `backend/app/`, not the live source — editing `backend/app/**` alone does nothing for AgentCore until that snapshot is refreshed, and the CLI reports success either way since it only knows about the copy. This is why the wrapper exists rather than a remembered manual step: it recurred twice (2026-08-17 first incident; confirmed recurring again live on 2026-08-25 before this fix — a prompt/tracing fix was "redeployed" successfully both times but kept reproducing because the codezip copy was never re-staged). No exceptions, no "it's a small change so skip it" — and no calling `agentcore deploy`/`agentcore.cmd deploy` by hand "just this once."
