...
- [ ] **TODO (fixed and redeployed, not yet live-verified)** Live driver-chat testing (owner, hosted) caught the standard context-lock line `I need help with shipment X` mis-triggering `escalate_exception` and creating a real `OPEN`/`HIGH` escalation (`ESC-53B8A6EA0A37` for `SHP-D16-RAVI`). Root cause: `backend/app/assistant/prompts.py` read "...when the driver asks for help or escalation" as an independent trigger instead of gated by `NO_FEASIBLE_SLOTS`. Fixed 2026-08-17 06:35 IST: escalate_exception now requires an explicit escalate ask or a prior NO_FEASIBLE_SLOTS result; a bare "I need help with shipment X" is documented as context-only, zero writes. Backend units 86 passed (text-only prompt change; no new deterministic test possible for LLM tool-selection). **Redeployed 07:20 IST**: owner ran `agentcore.cmd deploy --yes` (Runtime `READY`); this agent rolled ECS Express (`setuhaul-api:latest` rebuilt/pushed, canary rollout `COMPLETED`, `/health/live` 200). Still TODO: an actual hosted chat turn confirming the fix, and cleanup of the stray escalation record — both via `docs/HOSTED_SMOKE_CHAT_SCRIPT.md` §1 step 3.
