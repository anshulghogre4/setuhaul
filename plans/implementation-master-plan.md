...
Last refreshed: 2026-08-17 04:10 IST (Sprint 3 exit gate remains **COMPLETE**; Sprint 4 **PLANNED** — fixed hosted-only AgentCore event-loop `TOOL_ERROR` on Driver-chat DB tools (`invoke_agent` sync `asyncio.run()` → native async entrypoint so the SDK's persistent worker loop is used instead of a fresh loop per invocation) and a second gap where Ops escalation-resolve remarks were accepted but never persisted (migration + `escalation_service.py` + `get_exception_status`); unit 84 passed, 3 skipped, import smoke PASS; **not yet live-verified**, pending owner's migration apply + ECS + AgentCore Runtime redeploy; gate not struck)
...
- [ ] **TODO (live verification pending ECS + AgentCore redeploy)** Fixed hosted AgentCore event-loop `TOOL_ERROR` on Driver-chat DB tools: `backend/app/assistant/agentcore_main.py` `invoke_agent` was a sync `def` wrapping `asyncio.run(_run_turn(...))`, so the Bedrock AgentCore SDK ran it in a threadpool and a brand-new event loop was created per invocation, while the process-level `db` asyncpg pool (`backend/app/db/session.py`) stayed bound to whichever loop first created it — matches the observed "fails once, retry succeeds" LangSmith pattern. Fixed by making `invoke_agent` a native `async def` entrypoint so the SDK's own persistent per-container worker loop (`bedrock_agentcore/runtime/app.py`, previously unused by this handler) schedules every invocation consistently. Also fixed a second gap found auditing Aman's `be62264`: Ops "Mark Resolved" resolution notes were accepted by the API but never persisted (no DB column) — added migration `20260817040000_escalation_resolution_note.sql`, threaded `resolution_note` through `resolve_escalation()`, and added it to the `get_exception_status` SELECT. Code-verified 2026-08-17 04:10 IST: backend unit 84 passed, 3 skipped; import smoke PASS. Migration applied live 2026-08-17 04:35 IST (direct PostgreSQL connection; verified via `information_schema.columns`; `supabase/CHANGELOG.md` has the full record). **Application fixes not yet live-verified** — needs the ECS `setuhaul-api` image redeployed (hosted backend today predates even `be62264`) and the AgentCore Runtime redeployed; owner performing the AWS redeploy separately.
