...
- [ ] **TODO (code + both redeploys done and artifact-verified; only live hosted-chat re-test remains)** Live driver-chat testing (owner, hosted) caught the standard context-lock line `I need help with shipment X` mis-triggering `escalate_exception` and creating a real `OPEN`/`HIGH` escalation (`ESC-53B8A6EA0A37` for `SHP-D16-RAVI`). Four stages: (1) 06:35 IST prompt-wording fix gating escalation on `NO_FEASIBLE_SLOTS`/explicit ask; (2) 07:20→07:35 IST found and closed an `agentcore/codezip/` staging gap that had shipped stale code despite `agentcore.cmd deploy --yes` reporting success; (3) 08:10 IST — live re-test still reproduced the bug even with the verified-deployed prompt fix (LLM misjudging intent), so added a structural safety net: `escalate_exception` is now a two-step confirm write mirroring `report_delay_or_update_eta` (`EscalateExceptionCommand.confirmed` defaults `True` for existing system/ops callers, unchanged; the driver-chat tool's arg defaults `False`, misfired calls now return `CONFIRMATION_REQUIRED` with zero DB writes); new test asserts zero `session.execute` calls when unconfirmed; backend units 87 passed. (4) **08:40 IST** — both AgentCore (`agentRuntimeVersion=7`) and ECS (task-def `:9`) redeployed, and this time verified by downloading and grepping the actual deployed artifacts (not just CLI status, per the lesson from stage 2) — confirm-gate code confirmed present in both. Still TODO: an actual hosted chat turn confirming the confirm-gate holds live, and cleanup of the stray escalation record — via `docs/HOSTED_SMOKE_CHAT_SCRIPT.md` §1 step 3.
